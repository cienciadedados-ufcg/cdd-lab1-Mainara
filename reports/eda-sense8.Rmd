---
title: "Dados sobre séries de TV"
output:
  html_notebook:
    toc: yes
    toc_float: yes
  html_document:
    df_print: paged
    toc: yes
    toc_float: yes
---

```{r setup, echo=FALSE, warning=FALSE, message=FALSE}
library(tidyverse)
library(here)
theme_set(theme_bw())
```

Analisaremos os episódios das séries Black Mirror, Breaking Bad e Orphan Black.

```{r}
episodes = read_csv(here("data/series_from_imdb.csv"), 
                    progress = FALSE,
                    col_types = cols(.default = col_double(), 
                                     series_name = col_character(), 
                                     episode = col_character(), 
                                     url = col_character(),
                                     season = col_character())) %>% 
    filter(series_name %in% c("Orphan Black", "Black Mirror", "Breaking Bad"))
```

### Qual das séries que você escolheu é mais bem avaliada no IMDB? A diferença é grande? Pequena? Muda de acordo com as temporadas da série? 

Para responder essas perguntas precisaremos analisar a avaliação feita pelos usuários para os episódios de cada série. 

>> Cuidado com o uso do sumário sem olhar os pontos: 

```{r}
episodes %>% 
    ggplot(aes(x = season, y = user_rating, color = series_name)) + 
    geom_jitter()

episodes %>% 
    ggplot(aes(x = season, y = user_rating, color = series_name)) + 
    geom_boxplot()
```


Nesse gráfico são utilizadas as medianas das avaliações das temporadas.

```{r}
data_median <- episodes %>%
    group_by(series_name, season) %>%
    summarise(median_serie = median(user_rating)) 
    

ggplot(data = data_median,
       aes(x = season,
           y = median_serie,
           color = series_name,
           group = series_name)) +
    geom_line() +
    geom_point()
```
Através desse gráfico é possível perceber que a série mais bem avaliada no IMDB é Breaking Bad. Para verificar se a diferença entre as outras é grande, iremos calcular a média dos pontos obtidos através do gráfico:<br/>

```{r}
data_median %>% 
    group_by(series_name) %>% 
    summarise(media = mean(median_serie)) %>% 
    ggplot(aes(x = series_name, y = media)) + 
    geom_col() + 
    coord_flip()
```

Dessa forma, concluímos que não há uma diferença muito grande com relação às outras séries.
Além disso, observamos que os pontos do gráfico que as avaliações estão variando de acordo com as temporadas.

>> Seria legal comentar tendem a aumentar, se não há padrão claro, etc.

>> Outra coisa que senti falta é que ao comparar só a média / mediana, deixamos de fora informações legais como "Os melhores episódios estão melhorando?" "Os piores de cada temporada estão piorando?" A resposta pode usar mais de uma medida.


### Qual das séries que você escolheu tem episódios de qualidade mais irregular segundo o IMDB? A diferença é grande? Pequena? 

>> A ideia foi legal, mas por que não calcular o desvio padrão considerando todos os episódios, e calcular com base nas medianas? Seria importante explicar essa escolha. 

Para obter essa resposta, vamos calcular o desvio padrão das medianas encontradas no gráfico anterior.<br/>
   - **Black Mirror**:
```{r}
medians_BM <- data_median %>%
    filter(series_name %in% c("Black Mirror"))
med_BM = medians_BM$median_serie
sd(med_BM)
```

   - **Breaking Bad**: 
```{r}
medians_BB <- data_median %>%
    filter(series_name %in% c("Breaking Bad"))
med_BB = medians_BB$median_serie
sd(med_BB)
```
   - **Orphan Black**: 
```{r}
medians_OB <- data_median %>%
    filter(series_name %in% c("Orphan Black"))
med_OB = medians_OB$median_serie
sd(med_OB)
```
Ou seja, Breaking Bad possui mais episódios de qualidade mais irregular segundo o IMDB, pois um alto desvio padrão indica que os pontos dos dados estão espalhados por uma ampla gama de valores. A diferença dessa entre as outras duas é grande, mas das outras duas entre si é pequena.

>> Geralmente filtrar em tabelas diferentes é desnecessário: 

```{r}
data_median %>%
    group_by(series_name) %>% 
    summarise(`Desvio padrão da mediana` = sd(median_serie))
```



### Qual das séries ou temporadas tem episódios mais sem graça? (Um episódio é sem graça se ninguém nem ama nem odeia ele)

Os boxplots contruídos para encontrar essa resposta foi montado com base no somatório da proporção de usuários que avaliaram o episódio com os valores 4, 5 ou 6. Nesse caso, estamos avaliando todos os episódios de cada série, então iremos descobrir qual das séries tem episódios mais sem graça.

```{r}
data_not_funny <- episodes %>%
    group_by(series_name, episode) %>%
    summarise(sum = r4+r5+r6) 

ggplot(data = data_not_funny,  aes(x = series_name, y = sum, color = series_name, group=series_name)) +
geom_boxplot()

```
Pela distribuição dos dados mostrados nos boxplots, obtemos o seguinte ranking de séries que tem episódios mais sem graça:<br/>
   **1º Black Mirror**<br/>
   **2º Orphan Black**<br/>
   **3º Breaking Bad**<br/>
Essa conclusão é feita com base na linha da mediana de cada boxplot, dessa forma, a proporção de usuários que avaliaram os episódios de Black Mirror com os valores 4, 5 ou 6 é a maior dentre as 3 séries.

### Episódios no início/fim da temporada são mais bem avaliados que os no meio? O que tende a ser melhor? Inícios ou finais de séries/temporadas?

```{r}
episodes %>% 
    filter(series_name %in% c("Black Mirror")) %>% 
    ggplot(aes(x = episode, y = user_rating)) + 
    geom_col() + 
    facet_wrap( ~ season) +
    coord_flip()
               
```

A quantidade de avaliações varia bastante. Exceto pelo primeiro episódio, os episódios da segunda temporada tem muito menos avaliações. Talvez porque os dados foram coletados logo depois que a segunda temporada foi lançada. 

>> Acho que não entendi o raciocínio, mas a pergunta mirava em saber se os episódios iniciais e finais de uma temporada tem `user_rating` mais alto que os demais. Acho que você foi em outra direção. 


## Qual a distribuição da avaliação dos episódios?

>> Por que sense8 sobreviveu aqui?

```{r}
episodes %>% 
    ggplot(aes(x = "Sense8", y = user_rating)) + 
    geom_jitter(width = .05) +
    ylim(5, 10) + 
    labs(x = "Episódios", y = "Avaliação no IMDB")
```

```{r}
episodes %>% 
    ggplot(aes(x = user_rating)) + 
    geom_histogram(binwidth = .5, fill = "pink", color = "black") + 
    geom_rug(alpha = .5, size = 1.2)

```

```{r}
sumarios = episodes %>% 
    summarise(mediana = median(user_rating), 
              media = mean(user_rating))

episodes %>% 
    ggplot(aes(x = user_rating)) + 
    geom_histogram(binwidth = .5, fill = "white", color = "black") + 
    geom_vline(xintercept = sumarios$media, color = "darkorange") + 
    geom_vline(xintercept = sumarios$mediana, color = "brown") 
```

### Qual é esse episódio pior avaliado? 

```{r}
episodes %>% 
    filter(user_rating == min(user_rating))
```


## Qual a pior/melhor temporada segundo o IMDB?

```{r}
episodes %>% 
    ggplot(aes(x = season, y = user_rating)) + 
    geom_boxplot(width = .5, outlier.color = NA) +   
    geom_jitter(width = .1, size = 2, alpha = .5, color = "red")
```


### Há padrões na avaliação ao longo do tempo?

```{r}
episodes %>% 
  ggplot(aes(x = season_ep, y = user_rating, color = season)) + 
  geom_line() + 
  geom_point() 
```

Correlações

```{r}
episodes %>% 
    group_by(season) %>% 
    summarise(correlacao_linear = cor(season_ep, user_rating, 
                                      method = "pearson"), 
              correlacao_kendall = cor(season_ep, user_rating, 
                                       method = "kendall"))

```

## Qual o episódio mais detestado? 

```{r}
episodes %>% 
    ggplot(aes(x = season_ep, y = r1, color = season)) + 
    geom_line() + 
    scale_x_continuous(breaks = 1:12)
```

### E amado? 

```{r}
episodes %>%
    ggplot(aes(x = season_ep, y = r10, color = season)) +
    geom_line() +
    scale_x_continuous(breaks = 1:12)
```

### Amor e ódio andam juntos?

Quais são episódios que estão entre os mais amados e mais odiados? 

```{r}
episodes =  episodes %>%
    mutate(
        rank_odiados = row_number(r1), # maior = mais odiado
        rank_amados = row_number(r10), # maior = mais amado
        amado_odiado = rank_amados * rank_odiados) # maior = mais amado E odiado
    
episodes %>%
    ggplot(aes(
        x = r1,
        y = r10,
        color = season,
        size = amado_odiado,
        label = season_ep
    )) +
    geom_point(alpha = .7) +
    geom_text(nudge_x = .001, nudge_y = -.01) 
```

```{r}
episodes %>% 
    arrange(-amado_odiado)
```

