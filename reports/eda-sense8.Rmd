...
Para responder essas perguntsa precisaremos analisar a avaliação feita pelos usuários para os episódios de cada série.
```{r}
...
geom_line(stat='identity') +
     geom_point()
```
Através desse gráfico é possível perceber que a série mais bem avaliada no IMDB é Breaking Bad, porém não é uma diferença muito grande. Fazendo a média dos pontos de cada série obtemos a média das avaliações sendo:
   - Black Mirror: (8.3+8.2+8.4)/3 = 8.29
   - Breaking Bad: (8.7+8.9+8.7+8.9+9.45)/5 = 8.93
   - Orphan Black: (8.4+8.6+8.55+8.65)/4 = 8.55
Além disso, observamos que essa avaliação muda de acordo com as temporadas de cada série.

## Qual das séries que você escolheu tem episódios de qualidade mais irregular segundo o IMDB? A diferença é grande? Pequena?

Para obter essa resposta, vamos calcular o desvio padrão da mediana de cada temporada para cada série.
...
```
Ou seja, Breaking Bad possui mais episódios de qualidade mais irregular segundo o IMDB. A diferença dessa entre as outras duas é grande, mas das outras duas entre si é pequena.
...
```
Black Mirror tem mais episódios sem graça.
